...
#define NUM_COLD 1 // num times to run cold start
...
void copy_vars(prob_vars * new_vars,prob_vars *vars, all_data * data){
...
Timings op_split_ctrl(prob_vars * vars, all_data * data, prox_data * p_data){
...
int main(int argc, char **argv)
{
	...
	Timings cold_start;
//	double tot_cold_st_time=0;
	for(int i=0;i<NUM_COLD;i++){	
		cold_start = op_split_ctrl(vars,data,p_data);
		//init_vars(vars,data);
		//tot_cold_st_time+=cold_start.total_time/NUM_COLD;
	}
//	printf("real time %6.6f\n",tot_cold_st_time);
	printf("COLD START:\n");
	printf("Iterations %i\n",(int)cold_start.itns);
	printf("Taking %4.2f ms\n",cold_start.total_time);
	printf("Average time to solve linear system: %4.2f ms\n",cold_start.lin_sys_time);
	printf("Average time to take prox step: %4.4f ms\n",cold_start.prox_time);

	Timings warm_start[NUM_WARM],average={0};
	
	printf("RUNNING %i WARM STARTS\n",NUM_WARM);
	prob_vars * new_vars = malloc(sizeof(prob_vars));
	init_vars(new_vars,data);
	srand(0);	
	for (int j=0;j<NUM_WARM;j++){
		perturb_data(data->x_init,data);
		copy_vars(new_vars,vars,data);
		warm_start[j] = op_split_ctrl(new_vars,data,p_data);
